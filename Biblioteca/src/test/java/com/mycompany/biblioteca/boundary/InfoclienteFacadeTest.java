 ...
 * @author Melissa
 */
public class InfoclienteFacadeTest {
    
   private static InfoclienteFacade infoClienteTest;
    private EntityManager emMock;
    private Infocliente infoClienteEntity;

    @BeforeClass
    public static void setUpBeforeClass(){
        infoClienteTest = new InfoclienteFacade();
    }
    
    @Before
    public void setUp(){
        emMock = createMock(EntityManager.class); 
        infoClienteTest.setEntityManager();
        infoClienteEntity = new Infocliente();
    }
    ...
    public void testCreate() {
        emMock.persist(infoClienteEntity);
        replay(emMock);
        infoClienteTest.create(infoClienteEntity);
        verify(emMock);
    }
    ...
    public void testEdit(){
        expect(emMock.merge(infoClienteEntity)).andReturn(infoClienteEntity);
        replay(emMock);
        infoClienteTest.create(infoClienteEntity);
        verify(emMock);
    }
    ...
    public void testRemove(){
        expect(emMock.merge(infoClienteEntity)).andStubReturn(infoClienteEntity);
        emMock.remove(infoClienteEntity);
        replay(emMock);
        infoClienteTest.remove(infoClienteEntity);
        verify(emMock);
    }
    ...
    public void testFind() {
        expect(emMock.find(Infocliente.class, infoClienteEntity.getClientId())).andReturn(infoClienteEntity);
        replay(emMock);
        assertEquals(infoClienteEntity, infoClienteTest.find(infoClienteEntity.getClientId()));
        verify(emMock);
    }
    ...
    public void testFindAll() {
        ...
        List<Infocliente> resultList = new ArrayList<Infocliente>();
        resultList.add(infoClienteEntity);
        resultList.add(new Infocliente());
        ...
        expect(criteriaQueryMock.from(Infocliente.class)).andReturn(rootMock);
        ...
        assertEquals(resultList, infoClienteTest.findAll());
