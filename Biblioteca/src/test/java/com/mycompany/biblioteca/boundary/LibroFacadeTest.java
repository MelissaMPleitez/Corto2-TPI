 ...
 * @author Melissa
 */
public class LibroFacadeTest {
    
   private static LibroFacade libroTest;
    private EntityManager emMock;
    private Libro libroEntity;

    @BeforeClass
    public static void setUpBeforeClass(){
        libroTest = new LibroFacade();
    }
    
    @Before
    public void setUp(){
        emMock = createMock(EntityManager.class); 
        libroTest.setEntityManager();
        libroEntity = new Libro();
    }
    ...
    public void testCreate() {
        emMock.persist(libroEntity);
        replay(emMock);
        libroTest.create(libroEntity);
        verify(emMock);
    }
    ...
    public void testEdit(){
        expect(emMock.merge(libroEntity)).andReturn(libroEntity);
        replay(emMock);
        libroTest.create(libroEntity);
        verify(emMock);
    }
    ...
    public void testRemove(){
        expect(emMock.merge(libroEntity)).andStubReturn(libroEntity);
        emMock.remove(libroEntity);
        replay(emMock);
        libroTest.remove(libroEntity);
        verify(emMock);
    }
    ...
    public void testFind() {
        expect(emMock.find(Libro.class, libroEntity.getId())).andReturn(libroEntity);
        replay(emMock);
        assertEquals(libroEntity, libroTest.find(libroEntity.getId()));
        verify(emMock);
    }
    ...
    public void testFindAll() {
        ...
        resultList.add(libroEntity);
        resultList.add(new Libro());
        ...
        expect(criteriaQueryMock.from(Libro.class)).andReturn(rootMock);
        ...
        assertEquals(resultList, libroTest.findAll());
